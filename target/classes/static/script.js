            ...
            data.forEach(student => {
                ...
                const row = `
                    <tr>
                        ...
                        <td>
                            <button class="btn-on" onclick="deleteStudent(${student.courseId})">
                                Delete
                            </button>
                        </td>
                    </tr>
...
function deleteStudent(courseId) {
    ...
    fetch("/Course/Delete/" + courseId, {   // ✅ UPDATED
        method: "DELETE"
    })
        .then(() => {
...
function search() {
    ...
    const input = document.getElementById("sear-ch").value.trim().toLowerCase();
    ...
    if (foundCourse) {
        window.location.href = `course.html?id=${foundCourse.id}`;
    } else {
